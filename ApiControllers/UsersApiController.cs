...
namespace StackFlow.ApiControllers
{
    ...
    [Route("api/users")]
    // By default, all actions in this controller require an authenticated user with the "Admin" role
    ...
    public class UsersApiController : ControllerBase
    {
        ...
        /// <summary>
        /// Admin action: Soft-deletes a user's account by setting IsDeleted to true.
        ...
        /// <summary>
        /// Admin action: Permanently deletes a user's account from the database.
        /// This action will also delete all comments made by the user, all projects created by the user (and their associated tickets),
        /// and reassign any tickets assigned to or created by the user to the deleting admin.
        /// Accessible only by Admin role. Use with extreme caution.
        /// </summary>
        /// <param name="id">The ID of the user to permanently delete.</param>
        /// <returns>204 No Content on success, 404 Not Found, 400 Bad Request, or 403 Forbidden.</returns>
        [HttpDelete("{id}/permanent")] // Route: /api/users/{id}/permanent
        public async Task<IActionResult> HardDeleteUser(int id)
        {
            ...
            if (currentAdminId == id)
            {
                return Forbid("You cannot permanently delete your own account via this endpoint. This is a highly destructive action."); // 403 Forbidden
            }
            ...
            try
            {
                ...
                if (adminRole == null)
                {
                    return StatusCode(500, "Admin role not found for ticket/project reassignment.");
                }
                ...
                if (adminUser == null)
                {
                    return StatusCode(500, "No active admin found to reassign tickets/projects. Cannot permanently delete user."); // 500 Internal Server Error
                }

                // 1. Reassign tickets created by this user
                foreach (var ticket in userToDelete.CreatedTickets)
                {
                    ticket.Created_By = adminUser.Id;
                }
                _context.Ticket.UpdateRange(userToDelete.CreatedTickets);

                // 2. Reassign tickets assigned to this user
                foreach (var ticket in userToDelete.AssignedTickets)
                {
                    ticket.Assigned_To = adminUser.Id;
                }
                ...
                // 3. Delete all comments made by this user
                _context.TicketComment.RemoveRange(userToDelete.TicketComments);

                // 4. Delete all projects created by this user (this will cascade delete associated tickets)
                _context.Project.RemoveRange(userToDelete.CreatedProjects);
                ...
                return NoContent(); // 204 No Content for successful deletion without returning content
            }
            catch (Exception ex)
            {
                // Log the exception for debugging purposes
                return StatusCode(500, $"Error permanently deleting user: {ex.Message}"); // 500 Internal Server Error
            }
        }
    }
}
